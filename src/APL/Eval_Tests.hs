  ...
  testGroup
    ...
    [ testCase "Add" $
      ...
      testCase "Let (shadowing)" $
        ...
        testCase "Test for lambda" $
        eval envEmpty (Apply (Let "a" (CstInt 10) (Lambda "b" (Mul (Var "a") (Var "b")))) (CstInt 5))
        @?= Right (ValInt 50),

        testCase "try catch without error" $
        eval envEmpty (TryCatch (Add (CstInt 40) (CstInt 60)) (CstInt 100))
          @?= Right (ValInt 100),

        testCase "try catch with error" $
        eval envEmpty (TryCatch (Add (CstInt 2) (CstInt 2)) (CstInt 4))
          @?= Right (ValInt 5)
